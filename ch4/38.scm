...
;; previous version times4 also work in call-by-need lang
;; but this version times4 will not work in call-by-value lang
;; it is a unterminal prog in call-by-value lang
;; let d = proc (x) (f (x x)) in this version times4 will
;; first eval in call-by-value lang, so it will introduce infinite call
...
(add-test! '(times4-prev "      let makerec = proc (f)
                     let d = proc (x)
                              proc (z) ((f (x x)) z)
                     in proc (n) ((f (d d)) n)
...
(add-test! '(times4-now "let makerec = proc (f)
                     let d = proc (x) (f (x x))
                     in (f (d d))
             in let maketimes4 = proc (f) proc (x)
                            if zero?(x)
