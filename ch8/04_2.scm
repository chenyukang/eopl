...
;;(print (scan&parse "let demo = proc(x : int) x in -((demo 1), 1)"))
;;(print scan&parse "(proc(x : int) -(x,1)  30)")
;;(print scan&parse "let demo = proc(y : int, x : int) y in 30")
(print (scan&parse "1"))
(run "1")
(run "let demo = proc(y : int, x:int) y in 1")
(run "let demo = proc(x : int, y : int) x in (demo 1)")
(run "let demo = proc(y : int) y in 30")
(run "let p = proc(x : int, y : int) x in 30")
(run-all)
