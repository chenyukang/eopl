(let ((time-stamp "Time-stamp: <2000-12-18 15:03:41 wand>"))
  (eopl:printf "7-1.scm: continuation-passing interpreter ~a~%"
...
;^        "Varassign not implemented in 7-1.scm"))
;^    (begin-exp (exp1 exps)
;^      (eopl:error 'eval-expression
;^        "Begin not implemented in 7-1.scm"))
      )))
...
(define-datatype continuation continuation?
  ...
  (test-cont
    ...
    (env environment?) (cont continuation?))
  (prim-args-cont (prim primitive?) (cont continuation?))
  (let-exp-cont
    (ids (list-of symbol?)) (env environment?)
    (body expression?) (cont continuation?))
  (eval-rator-cont
    (rands (list-of expression?))
    (env environment?) (cont continuation?))
  (eval-rands-cont
    (proc expval?) (cont continuation?))
  (eval-first-cont 
    (exps (list-of expression?))
    (env environment?) (cont continuation?))
  (eval-rest-cont 
    (first-value expval?) (cont continuation?)))
